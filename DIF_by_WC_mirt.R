...
item_content_full <- sqlQuery(db, sprintf("
...
ORDER BY QuestionID, IsCorrect
                         ", paste0(unique(responses_full$qbtbQuestionID), collapse = ",")))

forms <- unique(responses_full$AssessmentID)
...
for(i in 1:length(forms)){
  ...
  formid <- forms[i]
  responses <- filter(responses_full, AssessmentID == formid)
  ...
  item_content <- item_content_full %>%
  ...
  out_MG <- multipleGroup(data = as.data.frame(responses[, -c(1:2)]), group = group, 
                          ...
                          itemtype = "Rasch", technical = list(NCYCLES = 1000), method = "QMCEM")
